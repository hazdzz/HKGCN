...
def get_parameters():
    ...
    parser.add_argument('--model', type=str, default='hkgcn', help='graph neural network model')
    parser.add_argument('--gso_type', type=str, default='sym_renorm_lap', \
                        choices=['sym_renorm_lap', 'rw_renorm_lap'], \
                        help='graph shift operator, default as sym_renorm_adj, rw_renorm_adj as alternative')
    parser.add_argument('--t', type=float, default=3, help='the diffusion time, t > 0')   
    parser.add_argument('--K', type=int, default=50, help='K order, default as 2')
    ...
    if args.gso_type != 'sym_renorm_lap' and args.gso_type != 'rw_renorm_lap':
    ...
    if mode == 'tuning':
        ...
        t, K, lr, weight_decay = [*param.values()]
    else:
        if args.t <= 0:
            raise ValueError(f'ERROR: t has to be greater than 0.')
        else:
            t = args.t
        if args.K <= 1:
            raise ValueError(f'ERROR: The order K is smaller than 2!')
        else:
            K = args.K
        lr = args.lr
...
def process_data(device, dataset, gso_type, t, K):
    ...
    feature = utility.calc_hkgcn_feature(gso, feature, t, K)
